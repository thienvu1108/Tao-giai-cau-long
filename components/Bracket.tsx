...
const Bracket: React.FC<BracketProps> = ({ matches, hasThirdPlaceMatch, onUpdateScore, onUpdateMatchInfo, tournamentName, categoryName, isReadOnly }) => {
  ...
  const handleCaptureImage = async () => {
    ...
    try {
      // Tạm thời đặt zoom về 1 để chụp ảnh chuẩn nhất
      ...
      const canvas = await (window as any).html2canvas(bracketRef.current, {
        useCORS: true,
      ...
      link.download = `So_Do_${categoryName.replace(/\s+/g, '_')}.png`;
      link.href = image;
      link.click();
      ...
      alert("Lỗi khi chụp ảnh sơ đồ. Vui lòng thử lại.");
    } finally {
  ...
  return (
    <div className="h-full w-full flex flex-col relative overflow-hidden bg-slate-50">
      <div className="fixed top-24 left-8 z-[60] flex flex-col gap-2 print:hidden">
         ...
         <button onClick={() => setZoom(z => Math.max(z - 0.1, 0.4))} title="Thu nhỏ" className="w-10 h-10 bg-white border border-slate-200 rounded-xl shadow-lg flex items-center justify-center text-slate-600 hover:bg-slate-50 active:scale-95 transition-all"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 12H4" /></svg></button>
         <div className="mt-4 border-t border-slate-200 pt-4 flex flex-col gap-2">
           <button 
             onClick={handleCaptureImage}
             disabled={isCapturing}
             title="Chụp ảnh sơ đồ"
             className={`w-10 h-10 rounded-xl shadow-lg flex items-center justify-center transition-all active:scale-95 ${isCapturing ? 'bg-slate-100 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
           >
             {isCapturing ? (
